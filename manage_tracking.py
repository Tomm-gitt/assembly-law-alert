...
def normalize_action(value: str) -> bool:
    ...
    if value in {"exclude", "off", "false", "제외"}:
        return False
    if value in {"resume", "on", "true", "재개"}:
        return True
...
def main() -> int:
    ...
    if not bill_no:
        raise ValueError("의안번호(BILL_NO)가 비어 있습니다.")
    ...
    matched_entry["status_tracking_note"] = "manual"
    save_seen(seen)
