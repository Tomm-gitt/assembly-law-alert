...
- `.github/workflows/hub-apps-script-deploy.yml` : GitHub 소스를 Apps Script로 push하고, 선택적으로 기존 Web App 배포도 갱신
...
3. 워크플로가 라이브 프로젝트를 `clasp pull`하여 `apps-script/hub/`에 커밋
4. GitHub에서 `apps-script/hub/`에 `Code.gs`, `appsscript.json` 및 전체 `.gs` 파일이 들어왔는지 확인
5. 이후 HUB 수정은 GitHub 소스를 기준으로 작업
6. 수정 후 **Deploy HUB Apps Script** 실행
   - `deploy_webapp=false`: Apps Script 편집기 HEAD 소스만 갱신
   - `deploy_webapp=true`: HEAD push 후 기존 Web App deployment ID도 새 버전으로 갱신

## 운영 원칙

- 최초 `Sync` 완료 전에는 Deploy 워크플로를 실행하지 않는다.
- `Deploy`는 `appsscript.json`과 최소 5개 이상의 `.gs` 파일이 없으면 실패하도록 방어한다.
...
- 코드 변경 전 필요하면 Sync 워크플로를 먼저 실행해 라이브 소스를 GitHub와 맞춘다.
...
2. 필요한 코드 수정
3. GitHub commit/PR
4. Deploy workflow 실행
5. 필요 시 Web App deployment 갱신
