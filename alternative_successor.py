...
def _is_committee_alternative_bill(row: Dict, watched_law: str) -> bool:
    ...
    # 위원회 대안은 제목에 (대안)이 붙거나 제안자 유형이 위원회/위원장으로 식별되는 경우만 허용한다.
...
def fetch_candidate_alternatives(
    ...
    for page in range(1, MAX_RECEIPT_PAGES + 1):
        data = monitor.request_api(
            session,
            monitor.RECEIPT_API,
            {"pIndex": str(page), "pSize": str(monitor.PAGE_SIZE)},
        )
        ...
        # 접수 API는 최신순이라는 전제 하에 검색구간보다 오래된 페이지까지 내려오면 종료한다.
...
def find_successor_bill(
    ...
    if len(ranked) > 1:
        ...
        # 동일 법률의 위원회 대안 후보가 같은 우선순위로 둘 이상이면 오탐 방지를 위해 자동승계하지 않는다.
        if first_key == second_key:
            print(
                f"[WARN] 대안 후보 복수로 자동승계 보류: {original_no} / "
                f"{ranked[0].get('bill_no')}, {ranked[1].get('bill_no')}"
            )
            return None
...
def register_successor(
    ...
    successor_entry = {
        ...
        # 첫 조회에서 모든 과거 단계를 조용히 baseline 하지 않도록 자동승계 표식을 남긴다.
